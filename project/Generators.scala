...
object Generators extends AutoPlugin {
  ...
  object autoImport {
    val generateAttributeFactories =
      taskKey[File]("Generate AttributeFactories.scala file from scala-js-dom")
    val generateElementFactories = taskKey[File](
      "Generate ElementFactories.scala file from HTML Living Standard"
    )
    val generateEntityBuilders = taskKey[File](
      "Generate EntityBuilders.scala file from HTML Living Standard"
    )

    val attributeDefinition = taskKey[AttributeDefinition](
      "Load attribute definition from HTML living standard."
    )
    val propertyDefinition =
      taskKey[Map[PropertyName, Seq[(InterfaceName, Type, List[Mod])]]](
        "Load property definition from scala-js-dom"
      )
  }
  ...
  override def projectSettings = Seq(
    propertyDefinition := {
      ...
      val Seq(domModuleReport) = compileReport.modules.filter { moduleReport =>
        moduleReport.module.organization == "org.scala-js" && moduleReport.module.name
          .startsWith("scalajs-dom")
      }
      ...
      val parsedSource =
        try {
          ...
          Seq(
            parseFromJar("org/scalajs/dom/Element.scala"),
            parseFromJar("org/scalajs/dom/HTMLElement.scala"),
      ...
      } yield setterName -> (className.value, tpe, (mods.view ++ methodMods).collect {
    ...
    generateEntityBuilders := {
      ...
      val entityDefs =
        for ((EntityRefRegex(entityName), Entity(_, text)) <- entityMap.view)
          yield {
      ...
      """
      val file =
        (Compile / scalaSource).value / "org" / "lrng" / "binding" / "EntityBuilders.scala"
      val fileContent = Seq(
    ...
    generateAttributeFactories := {
      val AttributeDefinition(interfacesByTagName, interfacesByAttribute) =
      ...
      val (defs, propertyOnlyDefOptions) = sortedNames.view.map { attributeName =>
        ...
        val attributeSetters = for {
          ...
          q"""
              @inline implicit def ${Term.Name(evidenceName)}:
                AttributeSetter[${Type
            .Name(interfaceName)}, $attributeTermName.type] =
        ...
        val propertyConstructors = for {
          ...
          val mountPointBuilderTermName =
            Term.Name(
              s"mountPointBuilder_${tpe.syntax.replace('.', '_')}_$interfaceName"
            )
          q"""
      ...
      """
      val file =
        (Compile / scalaSource).value / "org" / "lrng" / "binding" / "AttributeFactories.scala"
      val fileContent = Seq(
    ...
    generateElementFactories := {
      ...
      val document =
        webClient.getPage[HtmlPage]("https://html.spec.whatwg.org/")
      val tagDefs =
        document
          .querySelectorAll("dl.element")
          .asScala
          .view
          .flatMap {
            elementDl =>
              val domInterface = elementDl
                .querySelectorAll(".idl dfn[id$=element]")
                .asScala match {
                case Seq(domInterfaceDfn) =>
                  domInterfaceDfn.asText
                case Seq() =>
                  elementDl
                    .querySelectorAll("dd:last-child code")
                    .asScala
                    .filter { code =>
                      Set(
                        "Uses",
                      ...
                      ).contains(code.getPreviousSibling.asText)
                    } match {
                    case Seq(code) =>
                      code.asText
                    case _ =>
              ...
              val Some(heading) =
                Iterator.iterate(elementDl)(_.getPreviousElementSibling).find {
                  element =>
              ...
              heading.querySelectorAll("dfn").asScala.view.map { elementDfn =>
                ...
                q"""
                object ${Term.Name(tagName)} extends ElementFactory[${Type
                  .Name(domInterface)}] {
      ...
      """
      val file =
        (Compile / scalaSource).value / "org" / "lrng" / "binding" / "ElementFactories.scala"
      val fileContent = Seq(
